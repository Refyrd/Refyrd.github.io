...
const profanityRu = [
    'хуй', 'хуя', 'хуе', 'хую', 'хуи', 'хуё', 'hui', 'huy', 'xuy', 'пизд', 'пиzd', 'пиzdа', 'pizd', 'нахуй', 'нахуя', 'похуй', 'nahui', 'poxuy',
    'бляд', 'бля', 'блять', 'блят', 'blyad', 'blya', 'ебал', 'ебат', 'ебет', 'ебут', 'ебаш', 'ёб', 'ebal', 'ebat', 'yob',
    'ебан', 'eban', 'ебаш', 'заеб', 'zaeb', 'наеб', 'naeb', 'объеб', 'выеб', 'выеб', 'доеб', 'отъеб',
    'пиздец', 'пизда', 'пизды', 'пизду', 'пiзд', 'пиздa', 'pizdec', 'pizdets',
    'гандон', 'гондон', 'gandon', 'gondon', 'мудак', 'муда', 'мyдак', 'mudak',
    'сучка', 'сука', 'сyка', 'suka',
    'долбоеб', 'долбоёб', 'долбаеб', 'dolboeb',
    'хуесос', 'хуила', 'хуило', 'huiesos',
    'залупа', 'залуп', 'zalupa', 'шлюха', 'шлюх', 'shlyuha',
    'манда', 'мандa', 'мандов', 'manda',
    'пидор', 'пидар', 'пидр', 'pidor', 'pidar', 'pidr', 'петух', 'петуша',
    'разъеб', 'разьеб', 'раzzеб', 'razeb',
    'уебищ', 'уёбищ', 'uebish',
    'хер', 'хера', 'хрен', 'хрена', 'her', 'xer',
    'член', 'члена',
    'сперма', 'сперм',
    'гей', 'геи', 'геев',
    ...
    'ахуе', 'ахуи', 'ахуен',
    'приеб', 'приєб',
    'срать', 'сру', 'срет', 'срал', 'srat',
    'говно', 'гавно', 'говн', 'govno',
    'какать', 'какаш',
    'ссать', 'ссыт', 'ссал',
    'моча', 'мочой', 'мочи',
    'ссанина', 'говнина',
...
const _lat2Cyr = { 'a':'а','e':'е','o':'о','p':'р','c':'с','y':'у','x':'х' };
...
function hasProfanity(text) {
  return text.split(/\s+/).some(t => {
    let w = _cleanToken(t);
    if (w.length > 0 && _profSet.has(w)) return true;
    w = w.replace(/[aeopcyx]/g, c => _lat2Cyr[c]);
...
function censorProfanity(text, replacement = '***') {
  return text.replace(/\S+/g, t => {
    let w = _cleanToken(t);
    if (w.length > 0 && _profSet.has(w)) return replacement;
    w = w.replace(/[aeopcyx]/g, c => _lat2Cyr[c]);
    if (w.length > 0 && _profSet.has(w)) return replacement;
    return t;
  });
